...
# Extract the ORIGINAL script blocks
m_orig = list(re.finditer(r'(<script>)(.*?)(</script>)', orig, re.DOTALL))
script_orig = m_orig[-1].group(2) if m_orig else ''

# Extract everything up to the first custom script block in the current page
m_curr = list(re.finditer(r'<script>', curr))
first_custom_script_start = m_curr[-1].start() if m_curr else len(curr)

html_before_script = curr[:first_custom_script_start]
...
escaped_combined = re.sub(r'[^\x00-\x7F]', escape_non_ascii, combined)

final_html = html_before_script + '<script>\n' + escaped_combined + '\n</script>\n</body>\n</html>\n'

with open(curr_path, 'w', encoding='utf-8') as f:
    f.write(final_html)

print("HTML guardado con éxito y JS unificado correctamente.")
